 ...
 * Fixtures table (one column per gameweek, one row per held club).
 * Color/difficulty comes straight off the fixture's own projPts (already
 * the Points Projection Engine's output, computed server-side in
 * presenters.ts) — never a second, independently-invented difficulty
 * read, so a favorable-looking pill and a high projected-points number
 * always tell the same story.
 */
export function FixturePill({ index, fixture, T }: { index: number; fixture: ClubFixture | undefined; T: ThemeTokens }) {
  const gw = `GW${index + 1}`;
